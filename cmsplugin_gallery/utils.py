...
#    ]
    ...
    if 'django.template.loaders.app_directories.Loader' in settings.TEMPLATE_LOADERS:
        for app in settings.INSTALLED_APPS:
            _ = __import__(app)
            dir_ = os.path.dirname(_.__file__)
            if dir_ not in dirs_to_scan:
                # append 'templates' for app directories
                dirs_to_scan.append(os.path.join(dir_, 'templates'))
    ...
    if 'django.template.loaders.filesystem.Loader' in settings.TEMPLATE_LOADERS:
        for dir_ in settings.TEMPLATE_DIRS:
            if dir_ not in dirs_to_scan:
                ...
                dirs_to_scan.append(dir_)

    for dir_ in dirs_to_scan:
        found = glob.glob(os.path.join(dir_, 'cmsplugin_gallery/*.html'))
        for file in found:
            dir_, file = os.path.split(file)
            key, value = os.path.join(dir_.split('/')[-1], file), file
            f = False
